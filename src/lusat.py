...
def handler(event, context):
    ...
    accounts = [{'account_id': x, 'role_assume': y}
                for x, y in accounts_list.items()]

    for args in accounts:
        print(str(datetime_now) + ' cloud-lusat running for: '+str(args))
        SGCollector(args)
        ENICollector(args)
        VPCCollector(args)
        EC2Collector(args)
        print(str(datetime_now) + ' cloud-lusat DONE for: '+str(args))
