...
log = logging.getLogger("cloud-lusat-inventory-analizer")
    ...
    try:
        ...
        for i in eni_dict['NetworkInterfaces']:
            try:
                ...
                log.info(str(datetime_now) +
                         " no sgs found into the EC2: "+str(e))
        ...
        for i in elb_dict['LoadBalancerDescriptions']:
            try:
                ...
                log.info(str(datetime_now) +
                         " no sgs found into the ELB: "+str(e))
        ...
        for i in elb2_dict['LoadBalancers']:
            try:
                ...
                log.info(str(datetime_now) +
                         " no sgs found into the ALB: "+str(e))
        ...
        for i in rds_dict['DBInstances']:
            try:
                ...
                log.info(str(datetime_now) +
                         " no sgs found into the RDS: "+str(e))
...
def vpc_informationLog(data, aws_region_name, session):
    try:
        log.info(str(datetime_now)+" starting the flowlog analizer")
        client = session.client('ec2', region_name=aws_region_name)
...
def lb_informationLog(data, aws_region_name, session):
    try:
        log.info(str(datetime_now)+" starting the LB_LOG analizer")
